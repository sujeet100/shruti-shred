...
def _scores(**over) -> ProducerScores:
    base = dict(structure=4, dynamics=4, climax=4, motif=4, hook=4, balance=4, independence=4, mood_fit=4)
    base.update(over)
...
def test_assess_streams_the_eight_criteria_as_event_scores():
    ...
    assert set(critique[0].scores) == {"structure", "dynamics", "climax", "motif",
...
def test_scores_are_bounded_1_to_5():
    try:
        ProducerScores(structure=6, dynamics=3, climax=3, motif=3,
